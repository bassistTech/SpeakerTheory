...
C_eff_sealed = 1/(Math.pow(S_d, 2)*Math.pow(c, 2)*rho/V_box + 1/C_ms)
C_eff_ported = 1/(Math.pow(S_d, 2)*Math.pow(c, 2)*Math.pow(omega, 2)*rho/(V_box*(Math.pow(omega, 2) - Math.pow(w_port, 2))) + 1/C_ms)
if (Fport == 0) {
      C_eff = C_eff_sealed
      }
      else {
...
port_cone_frac = S_d*Math.pow(w_port, 2)/(S_port*(Math.pow(omega, 2) - Math.pow(w_port, 2)))
